...
- master节点至少3个，建议为奇数
- 多master集群需要准备一个负载均衡器，这里我们使用keepalived + haproxy来搭建高可用负载均衡器
...
55.192.0.7执行：  hostnamectl set-hostname master2
55.192.0.159执行：hostnamectl set-hostname master3
...
# 在master1执行
# 打开lb.sh脚本文件，修改配置信息
# VIP="55.192.0.1" 虚拟IP地址
# interface="ens0" 实际的网卡名称
# state="MASTER"两台负载均衡器，一台为MASTER，一台为BACKUP
# master节点数组  hostname#ip，masters=("master1#55.192.0.125" "master2#55.192.0.7" "master3#55.192.0.159")
chmod 777 lb.sh
./lb.sh
...
# VIP="55.192.0.1" 虚拟IP地址
# interface="ens0" 实际的网卡名称
# state="BACKUP"两台负载均衡器，一台为MASTER，一台为BACKUP
# master节点数组  hostname#ip，masters=("master1#55.192.0.125" "master2#55.192.0.7" "master3#55.192.0.159")
chmod 777 lb.sh
./lb.sh
...
# 添加其余两台master（第一台master init后会输出join命令，或者可以在第一台master执行sudo kubeadm token create --print-join-command获取到join命令），--control-plane表示该节点为master节点
...
kubeadm join 55.192.0.1:6553 --token prsjur.lnaih6vr385ez1of --discovery-token-ca-cert-hash sha256:16a7e60868d8a017de61e32e7aa45adf46343befd14d21fed3fe316999c66b1e 

# 执行完成后，在Master节点查看节点状态，此时节点状态都是NotReady
kubectl get node
~~~
...
# 修改配置，将default_runtime_name = “runc" 修改为：default_runtime_name = “nvidia"
